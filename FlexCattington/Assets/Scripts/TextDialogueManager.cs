...
public class TextDialogueManager : MonoBehaviour {
    ...
    public Dictionary<int, TextNode> nodes = new Dictionary<int, global::TextNode>();
        ...
        foreach (string s in sections)
        {
            ...
            if (responses.Length > 1)
            {
                ...
                for (int i = 1; i < responses.Length; i++)
                {
                    ...
                    {
                        scoreEffect = int.Parse(score[1]),
                        text = score[0],
                        resultID = int.Parse(score[2])
                    };
                    nodeS.options.Add(opt);//used IDs as a way to point to destination even if it doesn't exist yet 
